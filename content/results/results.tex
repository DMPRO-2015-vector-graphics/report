...
\subsection{SRAM}
Both the SRAM for containing instruction memory and the SRAM intended as a frame buffer were not used. Since we never implemented HDMI, there was no point using a frame buffer either. There were problems with the instruction memory SRAM. The FPGA didn't receive any data from the SRAM for some reason. However, the internal block RAM in the FPGA were large enough to hold the scene. 
