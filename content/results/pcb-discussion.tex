...
By introducing a transistor, and making the signal active low on the \gls{mcu} side, we managed to make the \gls{mcu} notice when configuration was complete.
...
After further studying of the \gls{fpga} configuration manual, we saw that this guide was different from what we intended in our design, so there was need for this extra \(V_{CC}\) connection. The guide we studied is on page 42 in \cite{\gls{jtag}-configuration}
